...
# Usage : ./create_new_user.sh <username> <password> <ip>
# Ex: ./create_new_user.sh client42 superpass 10.42.0.94
...
# 1. Créer l'utilisateur système et ses dossiers
echo "[+] Création de l'utilisateur Linux : $USER"
sudo useradd -d "$USERROOT" -s /sbin/nologin "$USER" || true
...
# 2. Appliquer quota utilisateur : 25 Mo (en blocs de 1K)
echo "[+] Application du quota de 25 Mo"
sudo setquota -u $USER 25600 25600 0 0 /srv/clients
